...
// Client-side filtering UI for /search. Hoisted out of page.tsx so the
// route can be a server component that fetches the permitted-river-ID
// set once at request time and passes it down. Permit badge rendering
// is otherwise impossible without either a per-row fetch (slow) or a
// stale static list.
...
interface Props {
  // Set is serializable across the server/client boundary as a plain
  // array; the server route passes it as string[] and we rebuild the
  // Set here for O(1) lookup inside the result loop.
  permittedRiverIds: string[]
}

export default function SearchClient({ permittedRiverIds }: Props) {
  const [query, setQuery] = useState('')
  const [filter, setFilter] = useState<'all' | 'ww' | 'wild'>('all')

  const permitSet = useMemo(() => new Set(permittedRiverIds), [permittedRiverIds])
